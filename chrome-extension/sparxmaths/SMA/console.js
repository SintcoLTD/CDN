...
async function checkUser() {
	...
	if (response.documents.length == 0) {
		await contactDatabase('insertOne', 'users', 'user-data', {
			users: {}
		});
	...
	if (!(hash in users)) {
		...
		let newDocumentId = await (await contactDatabase('insertOne', 'answers', 'user-data', {
			answers: {}
		})).insertedId;
		...
		contactDatabase('updateOne', 'users', 'user-data', [{
			"users": users
		}, documentId]);
	}
	...
	response = await contactDatabase('findOne', 'answers', 'user-data', {
		"_id": {
			"$oid": users[hash].id
		}
	});
	...
	let userDocument = await (await contactDatabase('findOne', 'answers', 'user-data', {
		"_id": {
			"$oid": userDocumentId
		}
	})).document
...
async function sendAnswerToDatabase() {
	...
	if (correctStatusElement !== null) {
		...
		try {
			...
			contactDatabase('updateOne', 'answers', 'user-data', [{
				"answers": userData
			}, userDocumentId]);
		} catch (err) {
			console.log(err)
...
async function credits() {
	...
	console.log.apply(console, ["%c Brought to you by Sulaymaan & Najm Ajmal %c\ud83d\ude80 ", "color: #fff; background: #8000ff; padding:5px 0;", "color: #fff; background: #242424; padding:5px 0 5px 5px;"])
}
